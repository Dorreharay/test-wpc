...
function getOrderType(request, response) {
  pool.query(
    `SELECT * FROM companies WHERE ordertype = '${request.params.ordertype}' ORDER BY id ASC`,
    (error, results) => {
      if (error) {
        throw error;
      }
...
function createOrder(request, response) {
  const { index, guid, balance, phone, address, registered, name, email } = request.body;

  pool.query(
    'INSERT INTO companies (index, guid, balance, phone, address, registered, name, email) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',
    [index, guid, balance, phone, address, registered, name, email],
    (error, results) => {
      if (error) {
        throw error;
      }
...
function updateOrder(request, response) {
  ...
  const { index, guid, balance, phone, address, registered, name, email, ordertype } = request.body;
