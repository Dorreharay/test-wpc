...
class OrdersTable extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    const { getOrders, orderListTypeCurrent, projectName, editMode, isEdited } = this.props;

    if (
      prevProps.projectName !== projectName ||
      prevProps.orderListTypeCurrent !== orderListTypeCurrent ||
      isEdited === true
    ) {
      getOrders(orderListTypeCurrent);
    }
  ...
  handleCheck = (index) => {
    ...
    this.setState({
      checked: checked.includes(index) ? checked.filter((item) => item !== index) : [...new Set([...checked, index])],
    });
  };
  ...
  toggleEditTab = (editIndex) => {
    ...
    this.setState({
      ...
      checked: checked.includes(editIndex)
        ? checked.filter((item) => item !== editIndex)
        : [...new Set([...checked, editIndex])],
      formData: {
        id: editedRow.id,
  ...
  toggleEditedAlert = () => {
    this.setState({ toggleAlertAfterEditing: true });
    setTimeout(() => this.setState({ toggleAlertAfterEditing: false }), 3000);
  };
  ...
  toNextPage = () =>
    this.setState({
      page: this.state.page !== this.props.ordersList.length - 1 ? this.state.page + 1 : this.state.page,
    });

  render() {
    const { checked, editTabToggled, editIndex, page, formData, toggleAlertAfterEditing } = this.state;
    const {
      ordersList,
    ...
    return ordersList.length !== 0 ? (
      <div>
        ...
        <ul className={styles.orderTypes}>
          {orderTypes.map((orderType, index) => (
            <li
              key={index}
              className={styles[orderType.className]}
              onClick={() => chooseOrdersListType(orderType.typeDesc)}
            >
              {orderType.typeName}
            </li>
          ))}
                  ...
                  {checked.length > 0 && checked.length < ordersList[page].length ? (
                    <div className={styles.checkbox}>
                      <input
                        ...
                        id="checkbox"
                        name=""
                        checked={checked.length === ordersList[page].length}
                      />
              ...
              <tr key={index} style={{ background: checked.includes(index) ? '#fff5dc' : 'white' }}>
                <td>
                  {editMode ? (
                    ...
                    <DeleteIcon onClick={() => deleteOrder(order.id)} className={styles.checkbox} />
                  ) : (
                    <div className={styles.checkbox}>
                      <input
                        type="checkbox"
                        value="1"
                        onChange={(e) => this.handleCheck(index)}
                        id={index}
                        disabled={editMode}
                ...
                <td onDoubleClick={() => this.handleCheck(index)}>
                  <div className={styles.innerText} onClick={(e) => this.copyToClipboard(e, order.registered)}>
                    {order.registered}
                  </div>
                </td>
                <td onDoubleClick={() => this.handleCheck(index)}>
                  <div className={styles.innerText} onClick={(e) => this.copyToClipboard(e, order.index)}>
                    {order.index}
                  </div>
                </td>
                <td onDoubleClick={() => this.handleCheck(index)}>
                  <div className={styles.innerText} onClick={(e) => this.copyToClipboard(e, order.email)}>
                    {order.email}
                  </div>
                </td>
                <td onDoubleClick={() => this.handleCheck(index)}>
                  <div className={styles.innerText} onClick={(e) => this.copyToClipboard(e, order.phone)}>
                    {order.phone}
                  </div>
                </td>
                <td onDoubleClick={() => this.handleCheck(index)}>
                  <div className={styles.innerText} onClick={(e) => this.copyToClipboard(e, order.address)}>
                    {order.address}
                  </div>
                </td>
              </tr>
        ...
        {copiedToClickboard ? <Alert message="Скопійовано" /> : null}
        {toggleAlertAfterEditing ? <Alert message="Зміни збережено" /> : null}
        ...
        <EditTab
          ...
          toggleEditedAlert={this.toggleEditedAlert}
      ...
      <div>
        ...
        <div className={styles.notFound}>Немає доступних заявок</div>
      </div>
    );
  }
}
